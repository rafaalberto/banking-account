...
import static com.api.account.utils.HttpUtils.TEST_PORT;
...
public class TransactionResourceIntegrationTest {
  ...
  public void shouldDepositSuccessfully() {
    ...
    Transaction transaction =
        new Transaction(
            accountInserted.getId(),
            accountInserted.getId(),
            convertTwoDecimalPlace(new BigDecimal(1000)),
            DEPOSIT);
  ...
  private void updateBalance(final Account account, final TransactionContext transactionContext) {
    balanceDao.updateBalance(account, transactionContext);
  }

  private void deleteAccount(final Long accountId) {
