...
public class TransferServiceImpl implements TransactionService {
        ...
        transactionManager.executeInTransaction(
            transactionContext -> {
              ...
              Account accountReceiver =
                  accountService.findByIdWithLock(receiverId, transactionContext);

              verifyData(transaction);

              accountSender =
                  accountSender.withBalance(
                      withdraw(accountSender.getBalance(), transaction.getAmount()));
              accountReceiver =
                  accountReceiver.withBalance(
                      deposit(accountReceiver.getBalance(), transaction.getAmount()));

              balanceService.updateBalancesForTransfer(
                  accountSender, accountReceiver, transactionContext);
              return null;
            });
      }
    }
  ...
  private void verifyData(final Transaction transaction) {
    if (transaction.getAccountSenderId().equals(transaction.getAccountReceiverId())) {
      throw new BusinessException(
          HttpUtils.HTTP_BAD_REQUEST_STATUS, "Account Sender and Receiver must be different");
    }
